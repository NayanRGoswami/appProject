...
function searched_post($search){
  $db_conn = new mysqli('localhost','londoners','London123!','Londoners');
   
    
	$qry_post = "select * from post_master where post_heading LIKE '%".$search."%' order by post_date DESC";
    ...
    $postid_url=$postid;
     // $likeNum=gethotposlikes($postid);
        ...
        echo "Sorry no such posts found.";
      }
    }
}
...
function getPostsByCategoryID($category_id){
  $db_conn = dbconnect(DBHOST, DBDB, DBUSER, DBPW);
	...
	$qry_post = "select * from post_master  pm
				  left outer join member_profile mp 
				  on pm.member_id =mp.member_id "				
                ." where pm.category_id =$category_id order by pm.post_date DESC limit 10";
...
//	$qry_post = "select * from post_master"		
  ...
  $qry_post = "select * from post_master  pm
    ...
    while($row=mysqli_fetch_array($rs))
    {
      ...
      $qry_post = "select count(post_thread_id)  total_comments from post_threads where post_master_id = ".$row["post_master_id"]  ;

      $rowCount = $db_conn->query($qry_post);
      
      
      
      $rowValue =mysqli_fetch_array($rowCount);
      //var_dump($rowValue);
      $total_comments = $rowValue[0];
   ...
   " <header class='clearfix'>".
           ...
           " <div class='post-meta clearfix'>".
                  ...
                  " <span class='category'><a href='category_post?id=".$category_id."' title='View all posts in Server &amp; Database'></a></span>".
                   " <span class='comments'> ".$total_comments." Comments</span>".
...
//         echo $row['member_id'];
        if ($_SESSION['userid'] == $row['member_id']) { 
           
          ?> 
						 
            <b>
           <a href = #dvContainer class ='btn-link' onclick ="currentPage(this,'post_all.php');" id = <?php echo 'D'.$row['post_master_id'];?> name =  <?php echo 'D'.$row['post_master_id'];?> >Delete </a> &nbsp;
                               
           <a href = #dvContainer  class ='btn-link'  onclick ="currentPage(this,'post_all.php');" id = <?php echo 'A'.$row['post_master_id'];?> name =  <?php echo 'A'.$row['post_master_id'];?> >Archive </a> </li>
               </b>                 <?php //}
    }
...
function getpaginateAllPostsByCategoryID($category_id,$offset,$no_of_records_per_page)
{
...
//	$qry_post = "select * from post_master"		
  ...
  $qry_post = "select * from post_master  pm
 				  left outer join member_profile mp
 				  on pm.member_id =mp.member_id"				
                ." where pm.category_id =".$category_id." and pm.post_active <>'D' order by pm.post_date DESC LIMIT $offset,$no_of_records_per_page";
