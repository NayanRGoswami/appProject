...
function comment_show($id){
        $db = new mysqli('localhost','root','data123!','Londoners');
                                                ...
                                                <div class="like-btn">
                                                        ...
                                                        <form>
                                                                ...
                                                                if(isset($_SESSION['adminid'])){
                                                                    //check manually if logged user type 3, a moderator
                                                                    $db = new mysqli('localhost','root','data123!','Londoners');

                                                                    $id=$_GET['id'];
                                                                    $query = "select user_type from user where user_id ='" . $_SESSION['adminid'] . "';";
                                                                    ...
                                                                    if($db->query($query) == true){
                                                                            ...
                                                                            if($rs->num_rows > 0){
                                                                                    while($row = $rs->fetch_assoc()){
                                                                                            if($row['user_type'] == '3'){
                                                                                                    if($posts[1]['post_active'] == 'B'){
                                                                                                            //display comment message and button to make post active again
                                                                                                            echo "<div class='support-widget' style='background: none; border:1px solid red;'>This post is not visible because: <br>" . $posts[1]['comments'] . "</div>";
                                                                                                            echo "<a class='like-it' onclick='activePost()' href='#' style='background: none; background-color: #188c22; padding-left:13px; color: #ffffff;'>Active post</a>";
                                                                                                    } else {
                                                                                                            //this is the delete button if the post_active is not equal to 'b'
                                                                                                            echo "<a class='like-it' data-toggle='modal' data-target='#modalDelete' href='#' style='background: none; background-color: #f22f2f; padding-left:13px; color: #ffffff;'>Delete post</a>";
                                                                                                    }
                                                                                            }  
                                                                                    }
...
<!--//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////-->
                        ...
                        function deletePost(){
                                var msm = document.getElementById('formComment').value;
                                if(msm === ""){
                                        ...
                                        var resp = function(response){
                                                if(response.status ==  "ok"){
                                                        ...
                                                        alert("Something bad happened and it was not possible to complete the task.");
                                                }
                                        }
                                        $.get("deletePost.php", { comment: document.getElementById('formComment').value, postid: <?php echo $_GET['id'] ?> })
                                                .done(resp);
                                }
                        }

                         //function to active post action
                         function activePost(){
                                var txt;
                                if (confirm("Do you wish to activate this post?")) {
                                        var resp = function(response){
                                                if(response.status ==  "ok"){
                                                        window.location.href = "single.php?id=" + <?php echo $_GET['id']; ?>;
                                                } else {
                                        ...
                                        $.get("activatePost.php", { postid: <?php echo $_GET['id'] ?> })
                                                .done(resp);
                                }
                         }
                                ...
                                <div class="modal-dialog modal-dialog-centered" role="document">
                                        <div class="modal-content">
                                                <div class="modal-header">
                                                        <h5 class="modal-title" id="exampleModalLongTitle">Inform a reason for why this post is being deleted</h5>
                                                </div>
                                                <div class="modal-body">
                                                        <form>
                                                        <div class="form-group">
                                                        <label for="formComment">Comment</label>
                                                        <textarea class="form-control" id="formComment" placeholder="type a comment" style="width: 95%;" rows="3"></textarea>
                                                        </div>
                                                        </form>
                                                </div>
                                                <div class="modal-footer">
                                                        <button id="btnClose" type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                                                        <button id="btnDelete" onclick="deletePost()" type="button" class="btn btn-primary">Delete</button>
                                                </div>
                                        </div>
                                </div>
	                </div>
